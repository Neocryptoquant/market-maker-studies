//! ============================================================================
//! INVOKERS MODULE - Cross-Program Invocation (CPI) Helpers
//! ============================================================================
//!
//! This file contains helper functions for calling OTHER programs from our AMM.
//!
//! WHAT IS CROSS-PROGRAM INVOCATION (CPI)?
//! On Solana, programs can call other programs. This is like one smart contract
//! calling another smart contract. It's how programs compose together.
//!
//! THINK OF IT LIKE:
//! - A restaurant calling a delivery service (outsourcing work)
//! - Your phone app calling the camera app (using another app's features)
//! - A function calling another function in a different library
//!
//! WHY DO WE NEED THIS?
//! The AMM needs to:
//! 1. **Transfer tokens** → Call SPL Token program
//! 2. **Mint/burn LP tokens** → Call SPL Token program
//! 3. **Place orderbook orders** → Call Serum/OpenBook program
//! 4. **Cancel orders** → Call Serum/OpenBook program
//! 5. **Settle trades** → Call Serum/OpenBook program
//!
//! WHAT'S IN HERE:
//! 1. **SPL Token Operations** - Token transfers, mints, burns
//!    - token_transfer() - Move tokens between accounts
//!    - token_mint_to() - Create new LP tokens
//!    - token_burn() - Destroy LP tokens
//!
//! 2. **Serum/OpenBook Operations** - Orderbook integration
//!    - invoke_dex_new_order_v3() - Place a new order
//!    - invoke_dex_cancel_order_v2() - Cancel an order
//!    - invoke_dex_settle_funds() - Collect filled order proceeds
//!    - invoke_dex_replace_order_by_client_id() - Update an order
//!
//! 3. **Authority Signatures** - Program Derived Addresses (PDAs)
//!    - Most functions use "with_authority" variants
//!    - These sign transactions on behalf of the AMM
//!
//! HOW IT WORKS:
//! ```
//! User swaps 100 USDC for SOL
//! ↓
//! Processor::process_swap_base_in()
//! ↓
//! Invokers::token_transfer_with_authority(
//!     source: user's USDC account
//!     destination: pool's USDC vault
//!     amount: 100 USDC
//!     authority: AMM authority (PDA)
//! )
//! ↓
//! SPL Token Program executes the transfer
//! ↓
//! Success! Tokens moved
//! ```
//!
//! KEY CONCEPTS:
//! 1. **invoke_signed()** - Call another program with authority
//! 2. **Program Derived Address (PDA)** - An address controlled by our program
//! 3. **Signer Seeds** - The "password" to use a PDA
//! 4. **Account Metas** - List of accounts the other program needs
//!
//! EXAMPLE: Minting LP Tokens
//! When a user deposits liquidity, we mint them LP tokens:
//! ```
//! Invokers::token_mint_to(
//!     mint: LP token mint account
//!     destination: user's LP token account
//!     authority: AMM authority (can mint because it owns the mint)
//!     amount: 100 LP tokens
//! )
//! ```
//!
//! EXAMPLE: Placing an Orderbook Order
//! When MonitorStep runs, we place orders on Serum/OpenBook:
//! ```
//! Invokers::invoke_dex_new_order_v3(
//!     market: SOL/USDC market
//!     side: Bid (buy order)
//!     price: $49.90
//!     quantity: 10 SOL
//!     authority: AMM authority (signs the order)
//! )
//! ```
//!
//! WHY "WITH_AUTHORITY" VARIANTS?
//! Some functions have two versions:
//! - `token_transfer()` - User signs (normal transfer)
//! - `token_transfer_with_authority()` - AMM signs (using PDA)
//!
//! The AMM needs the "with_authority" version because it's moving tokens
//! from accounts it controls (the vaults).
//!
//! ============================================================================
...
// ============================================================================
// INVOKERS STRUCT - Collection of CPI helper functions
// ============================================================================
// This struct doesn't hold any data - it's just a namespace for functions.

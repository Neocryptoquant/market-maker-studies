//! ============================================================================
//! INSTRUCTION MODULE - The AMM's Command System
//! ============================================================================
//!
//! This file defines ALL the commands (instructions) that users can send
//! to the Raydium AMM program.
//!
//! THINK OF IT LIKE:
//! - A restaurant menu (these are the dishes you can order)
//! - An API specification (these are the endpoints you can call)
//! - A remote control (these are the buttons you can press)
//!
//! WHAT'S IN HERE:
//! 1. Instruction data structures (what parameters each command needs)
//! 2. The AmmInstruction enum (all possible commands)
//! 3. Helper functions to build instructions
//! 4. Account lists for each instruction
//!
//! KEY INSTRUCTIONS:
//! - Initialize2: Create a new pool
//! - Deposit: Add liquidity (become an LP)
//! - Withdraw: Remove liquidity (stop being an LP)
//! - SwapBaseIn: Swap tokens (you know input amount)
//! - SwapBaseOut: Swap tokens (you know output amount)
//! - MonitorStep: Manage orderbook orders (the hybrid magic!)
//!
//! HOW IT WORKS:
//! 1. User builds an instruction (e.g., "Swap 100 USDC for SOL")
//! 2. User signs and sends transaction to Solana
//! 3. Solana calls our entrypoint with the instruction data
//! 4. We decode the instruction and call the appropriate processor function
//! 5. Processor executes the logic and updates state
//!
//! ============================================================================
...
// ============================================================================
// INITIALIZE INSTRUCTION - Create a new pool (DEPRECATED)
// ============================================================================
// This was the original initialization method. Use Initialize2 instead!

#[repr(C)]
#[derive(Clone, Copy, Debug, Default, PartialEq)]
pub struct InitializeInstruction {
    /// nonce used to create valid program address
    /// This is the "seed" for generating the pool's authority address
    pub nonce: u8,

    /// utc timestamps for pool open
    /// When should trading be allowed? (Unix timestamp)
    /// Used for scheduled launches (IDOs, fair launches)
    pub open_time: u64,
}

// ============================================================================
// INITIALIZE2 INSTRUCTION - Create a new pool (CURRENT METHOD)
// ============================================================================
// This is how you create a new AMM pool on Raydium.
//
// WHAT IT DOES:
// 1. Creates the pool account
// 2. Initializes vaults for both tokens
// 3. Creates the LP token mint
// 4. Sets up the orderbook integration
// 5. Deposits initial liquidity
//
// EXAMPLE:
// Create a SOL/USDC pool with 100 SOL and 5,000 USDC
// Initial price: $50 per SOL
...
pub struct InitializeInstruction2 {
    ...
    /// utc timestamps for pool open
    /// Set to 0 for immediate trading, or a future timestamp for scheduled launch
    pub open_time: u64,

    /// init token pc amount
    /// How much quote token (USDC) to deposit initially
    pub init_pc_amount: u64,

    /// init token coin amount
    /// How much base token (SOL) to deposit initially
    pub init_coin_amount: u64,
}

// ============================================================================
// PRE-INITIALIZE INSTRUCTION - Prepare for pool creation
// ============================================================================
// Optional step before Initialize2. Sets up some accounts in advance.
...
// ============================================================================
// MONITOR STEP INSTRUCTION - Manage orderbook orders
// ============================================================================
// This is RAYDIUM'S SECRET SAUCE! This is what makes it hybrid.
//
// WHAT IT DOES:
// 1. Plans new orders based on current pool state
// 2. Places orders on the Serum/OpenBook orderbook
// 3. Cancels old orders that are no longer needed
//
// WHO CALLS THIS:
// - "Crankers" (bots that help maintain the AMM)
// - Anyone can call it (it's permissionless)
// - Usually called every few seconds
//
// WHY IT'S NEEDED:
// The AMM can't do everything at once (compute limits).
// This instruction advances the state machine step by step.
//
// EXAMPLE:
// Pool price changed → Plan 7 new orders → Place them → Cancel 3 old ones

#[repr(C)]
#[derive(Clone, Copy, Debug, Default, PartialEq)]
pub struct MonitorStepInstruction {
    /// max value of plan/new/cancel orders
    /// How many orders to plan in this step (usually 7-10)
    pub plan_order_limit: u16,

    /// How many orders to place in this step (usually 7-10)
    pub place_order_limit: u16,

    /// How many orders to cancel in this step (usually 7-10)
    pub cancel_order_limit: u16,
}

// ============================================================================
// DEPOSIT INSTRUCTION - Add liquidity to the pool
// ============================================================================
// This is how you become a liquidity provider (LP).
//
// WHAT IT DOES:
// 1. Takes your tokens (SOL + USDC)
// 2. Adds them to the pool
// 3. Mints LP tokens for you (proof of ownership)
//
// EXAMPLE:
// You deposit: 10 SOL + 500 USDC
// You receive: 100 LP tokens
// Later you can burn those LP tokens to get your share back (plus fees!)
//
// IMPORTANT:
// - You must deposit both tokens in the current pool ratio
// - You can't just deposit SOL or just USDC
// - The pool will calculate the exact amounts needed
...
pub struct DepositInstruction {
    ...
    /// Maximum amount of base token (SOL) you're willing to deposit
    pub max_coin_amount: u64,

    /// Maximum amount of quote token (USDC) you're willing to deposit
    pub max_pc_amount: u64,

    /// Which side is the "base" for this deposit
    /// 0 = coin side, 1 = pc side
    pub base_side: u64,

    /// Minimum amount of the "other" token you'll accept
    /// Protects against price changes between building and executing the tx
    pub other_amount_min: Option<u64>,
}

// ============================================================================
// WITHDRAW INSTRUCTION - Remove liquidity from the pool
// ============================================================================
// This is how you exit your LP position.
//
// WHAT IT DOES:
// 1. Burns your LP tokens
// 2. Calculates your share of the pool
// 3. Sends you back SOL + USDC (proportional to your share)
//
// EXAMPLE:
// You have: 100 LP tokens (1% of total supply)
// Pool has: 10,000 SOL + 500,000 USDC
// You get back: 100 SOL + 5,000 USDC
//
// PLUS: All the fees you earned while being an LP!
...
pub struct WithdrawInstruction {
    ...
    /// How many LP tokens to burn
    pub amount: u64,

    /// Minimum amount of base token (SOL) you'll accept
    /// Protects against slippage
    pub min_coin_amount: Option<u64>,

    /// Minimum amount of quote token (USDC) you'll accept
    /// Protects against slippage
    pub min_pc_amount: Option<u64>,
}

// ============================================================================
// SET PARAMS INSTRUCTION - Update pool parameters
// ============================================================================
// Admin-only instruction to change pool settings.
...
// ============================================================================
// WITHDRAW SRM INSTRUCTION - Withdraw SRM fee discounts
// ============================================================================
// Serum/OpenBook offers fee discounts if you hold SRM tokens.
...
// ============================================================================
// SWAP BASE IN INSTRUCTION - Swap with known input amount
// ============================================================================
// "I have 100 USDC. How much SOL can I get?"
//
// WHAT IT DOES:
// 1. Takes your input tokens (100 USDC)
// 2. Calculates output using x × y = k formula
// 3. Checks minimum_amount_out (slippage protection)
// 4. Sends you the output tokens (SOL)
//
// EXAMPLE:
// amount_in: 100 USDC
// minimum_amount_out: 1.95 SOL (you'll accept no less than this)
// Actual output: 1.996 SOL ✓

#[repr(C)]
#[derive(Clone, Copy, Debug, Default, PartialEq)]
pub struct SwapInstructionBaseIn {
    // SOURCE amount to transfer, output to DESTINATION is based on the exchange rate
    /// How much you're swapping IN
    pub amount_in: u64,

    /// Minimum amount of DESTINATION token to output, prevents excessive slippage
    /// If you'd get less than this, the transaction fails
    pub minimum_amount_out: u64,
}

// ============================================================================
// SWAP BASE OUT INSTRUCTION - Swap with known output amount
// ============================================================================
// "I want exactly 10 SOL. How much USDC do I need to pay?"
//
// WHAT IT DOES:
// 1. Calculates input needed for desired output
// 2. Checks max_amount_in (you won't pay more than this)
// 3. Takes your input tokens
// 4. Sends you exactly the amount you wanted
//
// EXAMPLE:
// amount_out: 10 SOL (exactly what you want)
// max_amount_in: 505 USDC (you'll pay no more than this)
// Actual input: 500.25 USDC ✓
...
pub struct SwapInstructionBaseOut {
    ...
    /// Minimum amount of DESTINATION token to output, prevents excessive slippage
    /// Exactly how much you want to receive
    pub amount_out: u64,
}

...
//! ============================================================================
//!
//! Think of this file as the "Blueprint" for the AMM's memory.
//!
//! When you build a vending machine, you need to know:
//! - How many apples are inside?
//! - How many oranges are inside?
//! - What's the current price?
//! - Who owns this machine?
//! - What fees does it charge?
//!
//! This file defines ALL the data structures (like forms or templates) that
//! store this information on the blockchain. Every time someone swaps tokens,
//! adds liquidity, or checks the price, the program reads/writes to these
//! structures.
//!
//! KEY STRUCTURES IN THIS FILE:
//! 1. AmmInfo - The main "brain" - stores everything about the pool
//! 2. TargetOrders - Stores the orders the AMM wants to place on the orderbook
//! 3. Fees - Defines how much the AMM charges for swaps
//! 4. StateData - Tracks statistics (volume, fees collected, etc.)
//!
//! WHY IT MATTERS:
//! Without these structures, the AMM would have no memory. It wouldn't know
//! how many tokens it has, what price to charge, or who can withdraw funds.
//! ============================================================================
...
// ============================================================================
// CONSTANTS - Fixed values used throughout the AMM
// ============================================================================

// TEN_THOUSAND is used as a denominator for percentages
// Example: If fee is 25/10000, that means 0.25% fee
pub const TEN_THOUSAND: u64 = 10000;

// Maximum number of orders the AMM can place on the orderbook at once
// Think of this as "the AMM can only juggle 10 orders at a time"
pub const MAX_ORDER_LIMIT: usize = 10;

// ============================================================================
// EPOCH HELPER - Gets the current "time period" on Solana
// ============================================================================
// An "epoch" is like a chapter in Solana's history. Each epoch is ~2 days.
// The AMM uses this to track when certain events happened.

#[cfg(not(test))] // This version runs in production
pub fn get_recent_epoch() -> Result<u64, ProgramError> {
    use solana_program::{clock::Clock, sysvar::Sysvar};
    // Ask Solana: "What epoch are we in right now?"
    Ok(Clock::get()?.epoch)
}

#[cfg(test)] // This version runs during testing
pub fn get_recent_epoch() -> Result<u64, ProgramError> {
    use std::time::{SystemTime, UNIX_EPOCH};
    // For tests, we fake the epoch using current time
    Ok(SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
        .as_secs()
        / (2 * 24 * 3600)) // Divide by seconds in 2 days
}

// ============================================================================
// LOADABLE TRAIT - A helper to read/write data from blockchain accounts
// ============================================================================
// On Solana, data is stored in "accounts" (like files on a hard drive).
// This trait provides methods to safely read and modify that data.

pub trait Loadable: Pod {
    /// Load data from an account and get a mutable reference
    /// (Mutable = you can change it)
    fn load_mut<'a>(account: &'a AccountInfo) -> Result<RefMut<'a, Self>, ProgramError> {
        // TODO verify if this checks for size
        // Borrow the account's data and convert it to our struct type
        Ok(RefMut::map(account.try_borrow_mut_data()?, |data| {
            from_bytes_mut(data) // Convert raw bytes to our struct
        }))
    }

    /// Load data from an account and get a read-only reference
    fn load<'a>(account: &'a AccountInfo) -> Result<Ref<'a, Self>, ProgramError> {
        // Same as above, but read-only (can't modify)
        Ok(Ref::map(account.try_borrow_data()?, |data| {
            from_bytes(data)
        }))
    }

    /// Load data directly from a byte slice
...
// ============================================================================
// MACRO - A code generator that implements Loadable for our structs
// ============================================================================
// Instead of writing the same code over and over, this macro does it for us.
// It marks our structs as safe to convert to/from raw bytes.

macro_rules! impl_loadable {
    ($type_name:ident) => {
        unsafe impl Zeroable for $type_name {} // Can be filled with zeros
        unsafe impl Pod for $type_name {} // Plain Old Data (no pointers)
        unsafe impl TriviallyTransmutable for $type_name {} // Safe to transmute
        impl Loadable for $type_name {} // Can use our load methods
    };
}

// ============================================================================
// TARGET ORDER - A single order the AMM wants to place on the orderbook
// ============================================================================
// Remember: Raydium is special because it places orders on Serum/OpenBook.
// Each TargetOrder represents ONE limit order (like "Sell 10 SOL at $100 each")

#[cfg_attr(feature = "client", derive(Debug))]
#[repr(C, packed)] // Pack tightly in memory to save space
#[derive(Clone, Copy, Default)]
pub struct TargetOrder {
    pub price: u64, // The price for this order (in the market's format)
    pub vol: u64,   // The volume/amount for this order
}

// Mark TargetOrder as safe to convert to/from bytes
...
// ============================================================================
// TARGET ORDERS - The AMM's "order management system"
// ============================================================================
// This is where Raydium's magic happens! This structure stores:
// 1. What orders the AMM WANTS to place (buy_orders, sell_orders)
// 2. What orders are CURRENTLY placed (placed_x, placed_y)
// 3. How much inventory it's targeting (target_x, target_y)
//
// Think of it as the AMM's "to-do list" for the orderbook.

#[cfg_attr(feature = "client", derive(Debug))]
#[repr(C, packed)]
#[derive(Clone, Copy)]
pub struct TargetOrders {
    // Owner of this target orders account (the AMM pool)
    pub owner: [u64; 4], // Pubkey stored as 4 u64s (32 bytes total)

    // ========================================================================
    // BUY SIDE - Orders to buy the base token (e.g., buy SOL with USDC)
    // ========================================================================
    pub buy_orders: [TargetOrder; 50], // Up to 50 buy orders at different prices
    pub padding1: [u64; 8],            // Reserved space for future features

    // ========================================================================
    // INVENTORY TARGETS - What the AMM is aiming for
    // ========================================================================
    pub target_x: u128, // Target amount of base token (e.g., SOL)
    pub target_y: u128, // Target amount of quote token (e.g., USDC)

    // ========================================================================
    // PLANNING PHASE - What the AMM is calculating
    // ========================================================================
    pub plan_x_buy: u128,  // How much base token to buy
    pub plan_y_buy: u128,  // How much quote token needed to buy
    pub plan_x_sell: u128, // How much base token to sell
    pub plan_y_sell: u128, // How much quote token expected from selling

    // ========================================================================
    // EXECUTION TRACKING - What's actually happening
    // ========================================================================
    pub placed_x: u128, // Base token currently in placed orders
    pub placed_y: u128, // Quote token currently in placed orders

    // ========================================================================
    // PROFIT/LOSS TRACKING
    // ========================================================================
    pub calc_pnl_x: u128, // Calculated P&L for base token
    pub calc_pnl_y: u128, // Calculated P&L for quote token

    // ========================================================================
    // SELL SIDE - Orders to sell the base token (e.g., sell SOL for USDC)
    // ========================================================================
    pub sell_orders: [TargetOrder; 50], // Up to 50 sell orders at different prices
    pub padding2: [u64; 6],             // More reserved space

    // ========================================================================
    // ORDER REPLACEMENT - Managing order updates
    // ========================================================================
    // When the AMM needs to update an order, it cancels the old one and places a new one.
    // These arrays track which orders need to be replaced.
    pub replace_buy_client_id: [u64; MAX_ORDER_LIMIT], // Buy orders to replace
    pub replace_sell_client_id: [u64; MAX_ORDER_LIMIT], // Sell orders to replace

    // ========================================================================
    // ORDER SPACING - How far apart to place orders
    // ========================================================================
    // The AMM doesn't place all orders at the same price. It spreads them out.
    // These values control the spacing (as a fraction: numerator/denominator)
    pub last_order_numerator: u64,
    pub last_order_denominator: u64,

    // ========================================================================
    // STATE MACHINE CURSORS - Tracking progress through order management
    // ========================================================================
    // Because Solana has compute limits, the AMM can't do everything at once.
    // These cursors track "where we are" in the process.
    pub plan_orders_cur: u64,  // Current position in planning phase
    pub place_orders_cur: u64, // Current position in placing phase

    // ========================================================================
    // ORDER COUNTS - How many valid orders exist
    // ========================================================================
    pub valid_buy_order_num: u64,  // Number of active buy orders
    pub valid_sell_order_num: u64, // Number of active sell orders

    pub padding3: [u64; 10], // Even more reserved space

    // ========================================================================
    // FREE SLOTS - Bitmap tracking which order slots are available
    // ========================================================================
    // Each bit represents one slot. 1 = free, 0 = occupied.
    // This is a clever way to track 128 slots using just 128 bits (16 bytes).
...
// ============================================================================
// AMM STATUS - The pool's "operating mode"
// ============================================================================
// Just like a store can be "Open", "Closed", or "Remodeling", an AMM pool
// can be in different states that control what actions are allowed.
//
// WHY MULTIPLE STATES?
// - Safety: During emergencies, disable everything
// - Flexibility: Allow deposits but not swaps (e.g., during launch)
// - Upgrades: Transition between different operating modes
//
// COMMON LIFECYCLE:
// 1. Uninitialized → Pool doesn't exist yet
// 2. WaitingTrade → Pool created, waiting for launch time
// 3. SwapOnly → Normal operation (most pools stay here)
// 4. Disabled → Emergency shutdown

#[repr(u64)]
pub enum AmmStatus {
    /// Pool doesn't exist or hasn't been initialized yet
    Uninitialized = 0u64,

    /// Fully operational - all features enabled
    /// (Swaps, deposits, withdrawals, orderbook integration)
    Initialized = 1u64,

    /// Emergency mode - everything is disabled
    /// Used when there's a critical issue
    Disabled = 2u64,

    /// Users can only withdraw, nothing else
    /// Used when shutting down a pool gracefully
    WithdrawOnly = 3u64,

    /// Only deposits and withdrawals allowed
    /// No swaps, no orderbook orders
    /// Use case: Building up liquidity before enabling trading
    LiquidityOnly = 4u64,

    /// Deposits, withdrawals, and orderbook orders allowed
    /// But NO direct swaps
    /// Use case: Orderbook-only trading mode
    OrderBookOnly = 5u64,

    /// Deposits, withdrawals, and swaps allowed
    /// But NO orderbook orders
    /// Use case: Pure AMM mode (like Uniswap)
    SwapOnly = 6u64,

    /// Initial state after pool creation
    /// Automatically transitions to SwapOnly after open_time
    /// Use case: Scheduled token launches (IDO/fair launch)
    WaitingTrade = 7u64,
}

impl AmmStatus {
    /// Convert a number to an AmmStatus
    ...
    /// Convert an AmmStatus to a number (for storage)
    ...
    /// Can users add liquidity in this status?
    pub fn deposit_permission(&self) -> bool {
        match self {
            AmmStatus::Uninitialized => false, // Pool doesn't exist
            AmmStatus::Initialized => true,    // Yes
            AmmStatus::Disabled => false,      // Emergency mode
            AmmStatus::WithdrawOnly => false,  // Only withdrawals
            AmmStatus::LiquidityOnly => true,  // Yes, that's the point
            AmmStatus::OrderBookOnly => true,  // Yes
            AmmStatus::SwapOnly => true,       // Yes
            AmmStatus::WaitingTrade => true,   // Yes, building liquidity
        }
    }

    /// Can users remove liquidity in this status?
    pub fn withdraw_permission(&self) -> bool {
        match self {
            AmmStatus::Uninitialized => false, // Pool doesn't exist
            AmmStatus::Initialized => true,    // Yes
            AmmStatus::Disabled => false,      // Emergency mode
            AmmStatus::WithdrawOnly => true,   // Yes, that's the point
            AmmStatus::LiquidityOnly => true,  // Yes
            AmmStatus::OrderBookOnly => true,  // Yes
            AmmStatus::SwapOnly => true,       // Yes
            AmmStatus::WaitingTrade => true,   // Yes
        }
    }

    /// Can users swap tokens in this status?
    pub fn swap_permission(&self) -> bool {
        match self {
            ...
            AmmStatus::Disabled => false,      // Emergency mode
            AmmStatus::WithdrawOnly => false,  // Only withdrawals
            AmmStatus::LiquidityOnly => false, // Liquidity only
            AmmStatus::OrderBookOnly => false, // Orderbook only
            AmmStatus::SwapOnly => true,       // Yes, that's the point
            AmmStatus::WaitingTrade => true,   // Yes, after open_time
        }
    }

    /// Can the AMM place orders on the orderbook in this status?
    pub fn orderbook_permission(&self) -> bool {
        match self {
            ...
            AmmStatus::WithdrawOnly => false,  // Only withdrawals
            AmmStatus::LiquidityOnly => false, // Liquidity only
            AmmStatus::OrderBookOnly => true,  // Yes, that's the point
            AmmStatus::SwapOnly => false,      // Swaps only
            AmmStatus::WaitingTrade => false,  // Not yet
        }
    }
}
...
// ============================================================================
// Ensures a fraction (numerator/denominator) is valid
// Valid means: numerator < denominator and denominator != 0
// Example: 25/10000 is valid (0.25%), but 10000/25 or 25/0 is not
...
// ============================================================================
// FEES - The AMM's revenue model
// ============================================================================
// This structure defines HOW MUCH the AMM charges for different actions.
// All fees are stored as fractions (numerator/denominator) for precision.
//
// WHY MULTIPLE FEE TYPES?
// 1. swap_fee: What users pay when they swap (goes to liquidity providers)
// 2. trade_fee: Fee for orderbook trades (also goes to LPs)
// 3. pnl_fee: A cut of the profits from market making (goes to protocol)
// 4. min_separate: Minimum price difference for orderbook orders
//
// EXAMPLE:
// If swap_fee = 25/10000, that's 0.25%
// So swapping $1000 costs $2.50 in fees

#[repr(C, packed)]
#[derive(Clone, Copy, Debug, Default, PartialEq)]
pub struct Fees {
    // ========================================================================
    // MIN SEPARATE - Minimum spread for orderbook orders
    // ========================================================================
    // This controls how close together buy and sell orders can be placed.
    // Prevents the AMM from placing orders too close to each other.
    // Default: 5/10000 = 0.05%
    ...
    // ========================================================================
    // TRADE FEE - Fee charged on orderbook trades
    // ========================================================================
    // When someone fills one of the AMM's orderbook orders, this fee applies.
    // Goes to liquidity providers as compensation for providing liquidity.
    // Default: 25/10000 = 0.25%
    ...
    // ========================================================================
    // PNL FEE - Protocol's cut of market making profits
    // ========================================================================
    // When the AMM makes money from market making, the protocol takes a cut.
    // This is how Raydium (the protocol) earns revenue.
    // Default: 12/100 = 12%
    ...
    // ========================================================================
    // SWAP FEE - Fee charged on direct swaps
    // ========================================================================
    // When someone swaps directly against the AMM (not via orderbook),
    // this fee applies. Goes to liquidity providers.
    // Default: 25/10000 = 0.25%
...
impl Fees {
    ...
    /// Checks that all fractions are valid (numerator < denominator)
    ...
    pub fn initialize(&mut self) -> Result<(), AmmError> {
        // min_separate = 5/10000 = 0.05%
        // Orderbook orders must be at least 0.05% apart
        self.min_separate_numerator = 5;
        self.min_separate_denominator = TEN_THOUSAND;

        // trade_fee = 25/10000 = 0.25%
        // Orderbook trades cost 0.25%
        self.trade_fee_numerator = 25;
        self.trade_fee_denominator = TEN_THOUSAND;

        // pnl = 12/100 = 12%
        // Protocol takes 12% of market making profits
        self.pnl_numerator = 12;
        self.pnl_denominator = 100;

        // swap_fee = 25/10000 = 0.25%
        // Direct swaps cost 0.25%
...
// ============================================================================
// STATE DATA - Statistics and metrics tracking
// ============================================================================
// This structure tracks the pool's performance and history.
// Think of it as the pool's "dashboard" showing volume, fees, profits, etc.
...
pub struct StateData {
    ...
    // ========================================================================
    // The AMM makes money from market making. These track pending withdrawals.
    /// delay to take pnl coin
    /// Amount of base token profit waiting to be withdrawn by protocol
    pub need_take_pnl_coin: u64,

    /// delay to take pnl pc
    /// Amount of quote token profit waiting to be withdrawn by protocol
    pub need_take_pnl_pc: u64,

    /// total pnl pc
    /// Total quote token profit earned (all-time)
    pub total_pnl_pc: u64,

    /// total pnl coin
    /// Total base token profit earned (all-time)
    ...
    // ========================================================================
    /// ido pool open time
    /// Unix timestamp when trading is allowed to start
    /// Used for scheduled launches (IDOs, fair launches)
    ...
    /// Timestamp for transitioning between operating modes
    pub orderbook_to_init_time: u64,

    // ========================================================================
    // SWAP STATISTICS - Direction 1: Coin In → PC Out
    // ========================================================================
    // Tracks when users swap base token (SOL) for quote token (USDC)
    /// swap coin in amount
    /// Total base token swapped IN (all-time volume)
    pub swap_coin_in_amount: u128,

    /// swap pc out amount
    /// Total quote token swapped OUT (all-time volume)
    pub swap_pc_out_amount: u128,

    /// charge pc as swap fee while swap pc to coin
    /// Total quote token collected as fees (all-time)
    pub swap_acc_pc_fee: u64,

    // ========================================================================
    // SWAP STATISTICS - Direction 2: PC In → Coin Out
    // ========================================================================
    // Tracks when users swap quote token (USDC) for base token (SOL)
    /// swap pc in amount
    /// Total quote token swapped IN (all-time volume)
    pub swap_pc_in_amount: u128,

    /// swap coin out amount
    /// Total base token swapped OUT (all-time volume)
    pub swap_coin_out_amount: u128,

    /// charge coin as swap fee while swap coin to pc
    /// Total base token collected as fees (all-time)
    pub swap_acc_coin_fee: u64,
}

impl StateData {
    /// Initialize state data for a new pool
...
// ============================================================================
// AMM INFO - The "Master Control Panel"
// ============================================================================
// This is THE most important structure in the entire program.
// It contains EVERYTHING about a liquidity pool:
// - What tokens are in it?
// - How much of each token?
// - What fees does it charge?
// - What's its current status?
// - Where are the token vaults?
// - Where is the orderbook account?
//
// EVERY operation (swap, deposit, withdraw) reads and updates this structure.
//
// SIZE: 752 bytes (stored on-chain in a Solana account)
...
pub struct AmmInfo {
    ...
    // ========================================================================
    /// Initialized status.
    /// What mode is the pool in? (See AmmStatus enum)
    /// Common values: SwapOnly (6), WaitingTrade (7), Initialized (1)
    pub status: u64,
    ...
    /// token mint.
    ///
    /// SIMPLE EXPLANATION: This is a magic number used to create a unique
    /// wallet address that controls the pool's funds. It's like a seed.
    pub nonce: u64,
    ...
    // ========================================================================
    /// max order count
    /// How many orders to place on each side of the orderbook
    /// Default: 7 (so 7 buy orders + 7 sell orders = 14 total)
    pub order_num: u64,

    /// within this range, 5 => 5% range
    /// How wide of a price range to spread orders across
    /// Example: If price is $100, depth=5 means orders from $95 to $105
    pub depth: u64,
    ...
    // ========================================================================
    /// coin decimal
    /// How many decimals the base token has
    /// Example: SOL has 9 decimals (1 SOL = 1,000,000,000 lamports)
    pub coin_decimals: u64,

    /// pc decimal
    /// How many decimals the quote token has
    /// Example: USDC has 6 decimals (1 USDC = 1,000,000 micro-USDC)
    pub pc_decimals: u64,
    ...
    // ========================================================================
    /// amm machine state
    /// What phase of order management is the AMM in?
    /// (See AmmState enum: Idle, Planning, Placing, Canceling, etc.)
    pub state: u64,

    /// amm reset_flag
    /// Should the AMM reset its orderbook orders?
    /// (See AmmResetFlag enum: ResetYes, ResetNo)
    pub reset_flag: u64,
    ...
    // ========================================================================
    /// min size 1->0.000001
    /// Minimum order size (in normalized units)
    /// Prevents placing tiny orders that waste compute
    pub min_size: u64,

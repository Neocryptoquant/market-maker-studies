//! ============================================================================
//! PROCESSOR MODULE - The AMM's Brain
//! ============================================================================
//!
//! This is THE MOST IMPORTANT file in the entire program.
//! This is where ALL the magic happens!
//!
//! WHAT IS THIS FILE?
//! The processor is the "brain" that executes every instruction.
//! When a user sends a transaction, the entrypoint calls this processor,
//! and the processor does the actual work.
//!
//! THINK OF IT LIKE:
//! - A restaurant kitchen (where orders are actually prepared)
//! - A computer's CPU (where instructions are executed)
//! - A factory assembly line (where products are built)
//!
//! WHAT'S IN HERE:
//! 1. **Instruction Handlers** - Functions for each instruction type
//!    - process_initialize2() - Create a new pool
//!    - process_deposit() - Add liquidity
//!    - process_withdraw() - Remove liquidity
//!    - process_swap_base_in() - Swap with known input
//!    - process_swap_base_out() - Swap with known output
//!    - process_monitor_step() - Manage orderbook orders (THE HYBRID MAGIC!)
//!
//! 2. **Helper Functions** - Utilities used by handlers
//!    - check_accounts() - Validate all accounts
//!    - calc_take_pnl() - Calculate profit/loss
//!    - authority_id() - Generate program addresses
//!    - unpack_token_account() - Read token account data
//!
//! 3. **Orderbook Integration** - Raydium's secret sauce
//!    - plan_orders() - Calculate what orders to place
//!    - place_orders() - Put orders on Serum/OpenBook
//!    - cancel_orders() - Remove old orders
//!    - get_amm_orders() - Read current orders
//!
//! 4. **State Machine** - Managing the AMM's internal state
//!    - IdleState → PlanOrdersState → PlaceOrdersState → IdleState
//!    - This ensures orderbook operations happen step-by-step
//!
//! THE EXECUTION FLOW:
//! 1. Entrypoint receives transaction
//! 2. Entrypoint calls Processor::process()
//! 3. Processor decodes the instruction
//! 4. Processor calls the appropriate handler (e.g., process_swap_base_in)
//! 5. Handler validates accounts
//! 6. Handler performs calculations (using math.rs)
//! 7. Handler updates state (using state.rs)
//! 8. Handler calls other programs if needed (using invokers.rs)
//! 9. Result returns to entrypoint
//! 10. Transaction succeeds or fails
//!
//! WHY IS THIS FILE SO LARGE? (7000+ lines)
//! - Handles ALL instruction types (10+ different commands)
//! - Each instruction has complex validation logic
//! - Orderbook integration is sophisticated
//! - Safety checks everywhere (this handles real money!)
//! - Detailed error handling
//!
//! KEY CONCEPTS TO UNDERSTAND:
//! 1. **Account Validation** - Every account must be verified
//! 2. **Authority Checks** - Only authorized addresses can do certain things
//! 3. **Math Safety** - All calculations use checked arithmetic (no overflow!)
//! 4. **State Transitions** - The AMM moves between states carefully
//! 5. **Cross-Program Invocation** - Calling other programs (SPL Token, Serum)
//!
//! EXAMPLE: A Swap Transaction
//! ```
//! User: "Swap 100 USDC for SOL"
//! ↓
//! Entrypoint: "Call Processor::process()"
//! ↓
//! Processor: "Decode instruction → SwapBaseIn"
//! ↓
//! process_swap_base_in():
//!   1. Validate all accounts ✓
//!   2. Check pool status (SwapOnly?) ✓
//!   3. Calculate output: (10,000 × 100) / (500,000 + 100) = 1.996 SOL
//!   4. Check minimum_amount_out (slippage protection) ✓
//!   5. Transfer 100 USDC from user → pool
//!   6. Transfer 1.996 SOL from pool → user
//!   7. Update pool state (reserves, fees, volume)
//!   8. Emit logs
//! ↓
...
// ============================================================================
// TOKEN PROGRAM IDS - SRM and MSRM for fee discounts
// ============================================================================

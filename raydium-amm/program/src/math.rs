//! ============================================================================
//! MATH MODULE - The AMM's Calculator
//! ============================================================================
//!
//! This file contains ALL the mathematical operations for the AMM.
//! Think of it as the "calculator" or "recipe book" for the vending machine.
//!
//! WHAT'S IN HERE:
//! 1. Price calculations (x × y = k formula)
//! 2. Swap amount calculations (how much you get for a swap)
//! 3. Decimal conversions (SOL has 9 decimals, USDC has 6)
//! 4. Orderbook price/volume conversions
//! 5. Liquidity provider share calculations
//!
//! WHY SO MUCH MATH?
//! - Blockchains don't have floating-point numbers (no 1.5, 3.14, etc.)
//! - Everything must be integers (whole numbers)
//! - We use U128 and U256 for precision (very big integers)
//! - We must avoid overflow (numbers getting too big)
//! - We must avoid rounding errors (losing money due to rounding)
//!
//! KEY CONCEPTS:
//! - U128 = 128-bit unsigned integer (0 to 340 undecillion)
//! - U256 = 256-bit unsigned integer (even bigger!)
//! - Decimals: SOL = 1,000,000,000 lamports (9 decimals)
//! - Decimals: USDC = 1,000,000 micro-USDC (6 decimals)
//!
//! ============================================================================
...
// LARGE INTEGER TYPES
// ============================================================================
// Rust doesn't have built-in U256, so we construct it using a macro.
// These are used for calculations that might overflow u64 or u128.

// 256-bit unsigned integer (4 × 64-bit words)
// Used for calculations that need extreme precision
construct_uint! {
    pub struct U256(4);
}

// 128-bit unsigned integer (2 × 64-bit words)
// Used for most calculations (balance between precision and performance)
construct_uint! {
    pub struct U128(2);
}

// ============================================================================
// SWAP DIRECTION - Which way are you trading?
// ============================================================================
...
pub enum SwapDirection {
    ...
    /// Example: Swap USDC (pc) for SOL (coin)
    PC2Coin = 1u64,

    /// Input token coin, output token pc
    /// Example: Swap SOL (coin) for USDC (pc)
    Coin2PC = 2u64,
}

// ============================================================================
// ROUNDING DIRECTION - How to handle fractions
// ============================================================================
// Since we can't have 1.5 tokens, we must round to 1 or 2.
// The direction matters for fairness!
...
pub enum RoundDirection {
    ...
    /// Used when calculating how much to TAKE from user (be generous to user)
    Floor,

    /// Ceiling the value, ie. 1.9 => 2, 1.1 => 2, 1.5 => 2
    /// Used when calculating how much to GIVE to user (be conservative)
    Ceiling,
}

// ============================================================================
// CALCULATOR - The main math engine
// ============================================================================
// This struct contains all the static helper functions for calculations.
// It's like a toolbox of mathematical operations.
...
impl Calculator {
    // ========================================================================
    // TYPE CONVERSIONS - Safely convert between number types
    // ========================================================================

    /// Convert u64 to u128 (always safe, smaller to bigger)
    pub fn to_u128(val: u64) -> Result<u128, AmmError> {
        val.try_into().map_err(|_| AmmError::ConversionFailure)
    }

    /// Convert u128 to u64 (might fail if number is too big!)
    pub fn to_u64(val: u128) -> Result<u64, AmmError> {
        val.try_into().map_err(|_| AmmError::ConversionFailure)
    }

    // ========================================================================
    // POWER CALCULATION - For orderbook rebalancing
    // ========================================================================
    // This calculates how much "power" the AMM has to place orders.
    ...
    */
    ...
    // ========================================================================
    // SWAP CALCULATION - "Base In" (You know how much you're putting in)
    // ========================================================================
    // This is THE CORE AMM FORMULA: x × y = k
    //
    // SCENARIO: "I want to swap 100 USDC for SOL. How much SOL do I get?"
    //
    // THE MATH:
    // Before swap: coin × pc = k
    // After swap: (coin - amount_out) × (pc + amount_in) = k
    //
    // Solving for amount_out:
    // coin × pc = (coin - amount_out) × (pc + amount_in)
    // coin × pc = coin × pc + coin × amount_in - amount_out × pc - amount_out × amount_in
    // 0 = coin × amount_in - amount_out × pc - amount_out × amount_in
    // amount_out × (pc + amount_in) = coin × amount_in
    // amount_out = (coin × amount_in) / (pc + amount_in)
    //
    // SIMPLIFIED: amount_out = (reserves_out × amount_in) / (reserves_in + amount_in)
    //
    // EXAMPLE:
    // Pool has: 10,000 SOL and 500,000 USDC
    // You swap in: 1,000 USDC
    // You get out: (10,000 × 1,000) / (500,000 + 1,000) = 19.96 SOL

    pub fn swap_token_amount_base_in(
        amount_in: U128,                   // How much you're swapping IN
        total_pc_without_take_pnl: U128,   // Pool's quote token (USDC) reserves
        total_coin_without_take_pnl: U128, // Pool's base token (SOL) reserves
        swap_direction: SwapDirection,     // Which direction you're swapping
    ) -> U128 {
        let amount_out;
        match swap_direction {
            SwapDirection::Coin2PC => {
                // Swapping SOL (coin) for USDC (pc)
                // You're adding coin, taking out pc
            ...
            SwapDirection::PC2Coin => {
                // Swapping USDC (pc) for SOL (coin)
                // You're adding pc, taking out coin
    ...
    // ========================================================================
    // SWAP CALCULATION - "Base Out" (You know how much you want to get)
    // ========================================================================
    // This is the REVERSE of the above: x × y = k
    //
    // SCENARIO: "I want exactly 10 SOL. How much USDC do I need to pay?"
    //
    // THE MATH:
    // Before swap: coin × pc = k
    // After swap: (coin - amount_out) × (pc + amount_in) = k
    //
    // Solving for amount_in:
    // coin × pc = (coin - amount_out) × (pc + amount_in)
    // coin × pc = coin × pc + coin × amount_in - amount_out × pc - amount_out × amount_in
    // 0 = coin × amount_in - amount_out × pc - amount_out × amount_in
    // amount_out × pc + amount_out × amount_in = coin × amount_in
    // amount_out × (pc + amount_in) = coin × amount_in
    // pc + amount_in = coin × amount_in / amount_out
    // amount_in = (coin × amount_in / amount_out) - pc
    //
    // Wait, that's circular! Let's rearrange:
    // amount_in = (amount_out × pc) / (coin - amount_out)
    //
    // SIMPLIFIED: amount_in = (amount_out × reserves_in) / (reserves_out - amount_out)
    //
    // EXAMPLE:
    // Pool has: 10,000 SOL and 500,000 USDC
    // You want: 10 SOL
    ...
    //
    // NOTE: We use checked_ceil_div (round UP) to ensure the pool doesn't lose money!

    pub fn swap_token_amount_base_out(
        amount_out: U128,                  // How much you want to GET
        total_pc_without_take_pnl: U128,   // Pool's quote token (USDC) reserves
        total_coin_without_take_pnl: U128, // Pool's base token (SOL) reserves
        swap_direction: SwapDirection,     // Which direction you're swapping
    ) -> U128 {
        ...
        match swap_direction {
            SwapDirection::Coin2PC => {
                // You want USDC (pc) out, need to put SOL (coin) in
                ...
                amount_in = total_coin_without_take_pnl
                    ...
                    .unwrap()
                    .checked_ceil_div(denominator) // Round UP to protect the pool
                    .unwrap()
            }
            SwapDirection::PC2Coin => {
                // You want SOL (coin) out, need to put USDC (pc) in
